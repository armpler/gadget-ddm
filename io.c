...
#define SKIP  {my_fwrite(&blksize,sizeof(int),1,fd);}
  ...
  header.OmegaDm = All.OmegaDm;     //change
  header.OmegaLambda = All.OmegaLambda;
  header.HubbleParam = All.HubbleParam;

  header.OmegaR=All.OmegaR;            //change
  header.OmegaBaryon=All.OmegaBaryon; //change
...
void write_header_attributes_in_hdf5(hid_t handle)
{
  ...
  hdf5_dataspace = H5Screate(H5S_SCALAR);
  hdf5_attribute = H5Acreate(handle, "OmegaDm", H5T_NATIVE_DOUBLE, hdf5_dataspace, H5P_DEFAULT);
  H5Awrite(hdf5_attribute, H5T_NATIVE_DOUBLE, &header.OmegaDm);
  H5Aclose(hdf5_attribute);
  H5Sclose(hdf5_dataspace);
