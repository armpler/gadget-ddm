...
void check_omega(void)
{
  ...
  if(fabs(omega - (All.OmegaBaryon+All.OmegaDm) )> 1.0e-3) //change
    {
      if(ThisTask == 0)
	{
	  ...
	  printf
	    ("The mass content accounts only for Omega=%g,\nbut you specified Omega=%g in the parameterfile.\n",
	     omega, All.OmegaBaryon+All.OmegaDm);   //change
	  printf("\nI better stop.\n");
...
void seed_glass(void)
{
  ...
  partmass = (All.OmegaDm+All.OmegaBaryon) * (3 * All.Hubble * All.Hubble / (8 * M_PI * All.G))
