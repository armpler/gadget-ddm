...
void advance_and_find_timesteps(void)
{
  ...
  if(All.ComovingIntegrationOn)
    {
      ...
      hubble_a = hubble_function(All.Time);  //change
  ...
  dmean = pow(P[0].Mass / ((All.OmegaDm+All.OmegaBaryon) * 3 * All.Hubble * All.Hubble / (8 * M_PI * All.G)), 1.0 / 3);
  ...
  if(All.ComovingIntegrationOn)
      ...
      for(type = 0; type < 6; type++)
	{
	  if(count_sum[type] > 0)
		...
		dmean =
		  pow(min_mass[type] /
		      ((All.OmegaDm) * 3 * All.Hubble * All.Hubble / (8 * M_PI * All.G)),
		      1.0 / 3);
